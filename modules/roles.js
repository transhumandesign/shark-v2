...
module.exports.onCommand = async (message, command, args) => {
	...
	// get available roles
	const regional_roles = config.regional_roles.map(x => util.getRole(x)).filter(Boolean);
	const open_roles = config.open_roles.map(x => util.getRole(x)).filter(Boolean);
	const available_roles = [...regional_roles, ...open_roles];
	const available_role_names = available_roles.map(x => x.name);
	const available_role_names_sanitized = available_role_names.map(x => util.sanitize(x));
	...
	const can_add_role = available_roles.includes(role);
	if (!can_add_role) {
	...
	const msg = await message.channel.send('Organising roles...').catch(err => {
	...
	if (!util.userHasRole(message.author, role)) {
		const regional = regional_roles.includes(role);
		if (regional) {
			// remove all other region roles
			const roles = regional_roles.filter(x => util.userHasRole(message.author, x));
